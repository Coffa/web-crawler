	...
	module Strategies
		class VBulletinThree < Forum
			class << self
				...
				def parse(curl)
					begin
						get_info_first_link(curl, curl.html.at_css('table[id^=post]:first')) unless curl.behavior == :update || curl.parser_data.present?
						navigation_last(curl)
						get_info_last_link(curl, curl.html.at_css('table[id^=post]:last'))
